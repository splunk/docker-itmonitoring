...
/usr/bin/curl -sSk -H "Authorization: Bearer $TOKEN" https://$KUBERNETES_SERVICE_HOST:$KUBERNETES_PORT_443_TCP_PORT/api/v1/endpoints | jq '.items[] | [{metadata: .metadata, subsets: .subsets}]'
