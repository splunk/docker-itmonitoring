...
/usr/bin/curl -sSk -H "Authorization: Bearer $TOKEN" https://$KUBERNETES_SERVICE_HOST:$KUBERNETES_PORT_443_TCP_PORT/api/v1/pods| jq '.items[]| [{metadata: .metadata, spec: .spec, status: .status}]'
